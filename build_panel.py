...
def build(mkt):
    ...
    # 비용 = 거래세 + 슬리피지.
    # 거래세: 과거 실제 세율(0.30~0.18%)이 아니라 **현행 0.15% 로 통일**한다(2026-09-07 사용자 결정).
    #   앞으로 할 거래만 판단 대상이므로, 지금 세율로 재는 것이 실전 기대치에 가깝다.
    # 슬리피지: 신호 다음날 시가 매수는 갭·경쟁으로 원하는 가격에 못 산다. 유동성이 낮을수록 크다.
    #   2026-09-07 사용자 판단으로 기존 0.20~1.00% 를 **절반(0.10~0.50%)** 으로 낮췄다.
    #   0 으로 두지 않는 이유: 거래대금 10억 미만 종목에서 실제로 밀리기 때문이다.
    _tax = 0.15
    df["cost"] = _tax + np.select([df.amt20>=100, df.amt20>=50, df.amt20>=20, df.amt20>=10],
                                  [.10,.15,.25,.35], default=0.50)

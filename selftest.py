...
try:
    ...
    fd = f.get("date") or ""
    # ⚠ 여기서 견주는 last 는 **로컬 site/** 의 기준일이다. 개발 PC 에서는 그게 며칠 묵어
    #   있을 수 있어서, 앞선 경우(fd > last)를 유실로 단정하면 검사가 거짓말을 한다
    #   (2026-09-11 실제 오탐: 알림 20260911 · 로컬 site 20260910). 방향을 갈라서 본다.
    if fd == last:
        ok(f"알림 기준일이 최신 거래일과 같다 ({fd})")
    elif fd > last:
        warn(f"알림 기준일 {fd} 이 로컬 site/ ({last}) 보다 앞선다 — 로컬 빌드가 낡은 것이지 "
             f"유실이 아니다. 배포본으로 확인할 것")
    else:
        ng(f"알림 기준일 {fd or "없음"} < 최신 거래일 {last} — 그 사이 신규 편입 알림이 나가지 않았다")
